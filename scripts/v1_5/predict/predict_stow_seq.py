...
def get_args():
    ...
    parser.add_argument(
        "--input-file",
        type=str,
        default=os.path.join(LLAVA_PATH, "data/stow/test/dataset_seq.json"),
    )
...
if __name__ == "__main__":
    ...
    image_folder = os.path.join(LLAVA_PATH, "data/bin_real/images")
    dataset_json = json.load(open(args.input_file, "r"))

    res = []

    for x in tqdm(dataset_json):
        img_files = x["image"].split(",")
        img_files = [os.path.join(image_folder, img_file) for img_file in img_files]
        prompt = "<image> " * (len(img_files) - 1) + prompts["object_detection_xyxy_seq"]
        eval_args = type(
            "Args",
            (),
            {
